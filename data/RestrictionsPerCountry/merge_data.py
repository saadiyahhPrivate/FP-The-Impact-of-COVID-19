...
for date, group in grouped_data:
    ...
    for row_index, row in group.iterrows():
        ...
        if (len(curr_data) != 0):
            ...
            # if int(curr_data[-1]['Date']) == 20200323 and curr_data[-1]['CountryName'] == "Myanmar":
            #     print(row)

        # if int(row['Date']) == 20200323 and row['CountryName'] == "Myanmar":
        # if row['CountryName'] == "Myanmar":
        #     print("**** Myanmar row", row)
    ...
    for country in all_countries:
        if country not in seen_countries:
            if len(perCountry[country]):
                ...
                # perCountry[country].append(perCountry[country][-1])
                # perCountry[country][-1]['Date'] = date
                # seen_countries.append(country)

for el in perCountry['Italy']:
    print(el)

# print(perCountry['Myanmar'])
...
print(byDate[20200320])

with open('final_school_restrictions.json', 'w') as fp:
    json.dump(byDate, fp)

# for country in perCountry.keys():
#     print(perCountry[country])
